...
const EncounterDetailPage = () => {
    ...
    const fetchEncounter = async () => {
      ...
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('encounters').select('*').eq('id', id).eq('user_id', user.id).single();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('encounters').delete().eq('id', id).eq('user_id', user.id);
      if (error) throw error;
      toast({
        title: "Eliminato",
  ...
  return <div className="space-y-8 px-[16px] py-[12px]">
      ...
      <Card>
        <CardFooter className="pt-6 flex justify-between">
          <EncounterDetailActions encounterId={id as string} onDelete={handleDelete} isDeleting={isDeleting} />
        </CardFooter>
      </Card>
    </div>;
};
